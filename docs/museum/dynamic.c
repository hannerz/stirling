/* /secure/daemon/dynamic.c
 * Canland LPC Library
 *
 * 
 * Handles faction and town stats
 * 
 * This is the daemon that handles the addition and subtraction
 * of stats to towns and factions.  Although the code behind the 
 * system is pretty basic, as you can see, it is inherited and used by 
 * almost all objects, making it a very important part of the 
 * game, in the long run.
 * ~~~~~~~~~~~~~~
 *    080613 emsenn@Canland: created
 */
#include <lib.h>
#include <save.h>
#include <privs.h>
...
static void create() {
    daemon::create();
    if( !save_file ) save_file = "save/dynamic";
...
mixed GetFactionStat(string org, string type){
    return FactionStats[lower_case(org)][lower_case(type)];
}
...
/* Returns the value of org's score in type. */
mixed GetTownStat(string org, string type){
    return TownStats[lower_case(org)][lower_case(type)];
}
